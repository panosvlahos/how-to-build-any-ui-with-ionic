...
export class Tab4Page {
  ...
  constructor() {
    ...
    this.exampleForm.get('interests').valueChanges.subscribe(values => {
      this.submitSuccess = ''; // Reset on value change
    });
  }
  ...
  onSubmit() {
    ...
    this.exampleForm.value.interests.map((value: any, index: number) => {
      if (value) {
        selectedInterests.push(this.interestsList[index].name);
      }
    });
    this.submitSuccess = 'Submitted values: ' + selectedInterests.join(', ');
  }
}
